...
public class HttpData extends RetrofitUtils {
    ...
    public void HttpDataLogin(String username, String password, String cid, String macCode, Observer<String> token) {
        Observable observable = service.userLogin(username, password, cid, macCode).map(new HttpResultString<String>());
//        Observable observableCache=providers.getUserToken(observable)
        setSubscribe(observable, token);
    }
    ...
    private class HttpResultString<T> implements Func1<HttpResult<T>, T> {
        @Override
        public T call(HttpResult<T> httpResult) {
            if (!httpResult.isSuccess()) {
                throw new ApiException(httpResult);
